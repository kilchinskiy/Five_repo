...
class Record:  # Зберігає данні.
    ...
    def edit_phone(self, old_phone, new_phone):  # Старий номер на новий.
        # Спочатку знаходимо індекс старого номера
        for idx, p in enumerate(self.phones):
            if p.value == old_phone:
                # Спробуємо створити об'єкт Phone для валідації нового номера.
                # Якщо new_phone некоректний — тут буде піднято ValueError
                new_phone_obj = Phone(new_phone)
                # Заміна старого номера на валідний новий
...
def input_error(func):  # Декоратори.
    def inner(*args, **kwargs):
        try:
            return func(*args, **kwargs)
        except ValueError as e:  # Невірний формат.
            return str(e)
...
@input_error
def change_contact(args, book: AddressBook):  # Змінити телефон у контакта.
